...
- 编写 HTTP / JSON API / `async fn main` 服务端（无独立后端框架，见补丁二十）
...
- HTTP 服务：无 Express 级框架；`moon new` 默认 `wasm-gc`，服务端须改 `preferred_target = "native"` + `moonbitlang/async`（补丁二十）
- 数组模式至多一个 `..`；部分类型弃用 `Show` 插值，改用 `@debug.to_string` / `repr`（补丁二十一）
